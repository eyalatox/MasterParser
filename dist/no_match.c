...
sasl_callback_t *mgo_sasl_callbacks(const char *username, const char *password)
{
	...
	strcpy((char *)secret->data, password); // this is a very bad idea to have such allocation because it is not safe, it may cause buffer overflow which is a security issue. for example, if the password is longer than the allocated size, it will cause buffer overflow.
	secret->len = len;
